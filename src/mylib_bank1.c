...
Account* loadAccountsFromFile() {
    FILE* file = fopen("database/ambank.txt", "r");
    if (file == NULL) {
        return NULL;
    }
...
void saveAccountsToFile(Account* root) {
    ...
    FILE* file = fopen("database/ambank.txt", "a");
    if (file == NULL) {
...
void deposite(Account* currentAccount, Transaction** transactionHistory) {
    ...
    updateAccountBalance("database/ambank.txt", currentAccount->accountNumber, currentAccount->balance);
}
...
void transfer(Account* currentAccount, Transaction** transactionHistory) {
    ...
    if (choice == 'y' || choice == 'Y') {
        ...
        // updateAccountInFile("database/bankutama.txt", targetAccount);  // Update akun penerima di txt bank kedua
...
void updateAccountBalance(const char* filename, int accountNumber, float newBalance) {
        FILE* file = fopen(filename, "r+");
            ...
            FILE* tempFile = fopen("database/temp_ambank.txt", "w");
            if (tempFile == NULL) {
            ...
            fclose(tempFile);
            remove(filename);
            rename("database/temp_ambank.txt", filename);
        }
        ...
        switch (choice) {
            ...
            case 5:
                ...
                if (printChoice == 'y' || printChoice == 'Y') {
                    printTransactionsToFile(transactionHistory, "database/ambank-receipts.txt");
                }
                ...
                printf("============================================================================");
                gotoxy(33,17);
                printf("               Terima kasih telah menggunakan layanan AMBANK               ");
                gotoxy(33,18);
                printf("============================================================================");
